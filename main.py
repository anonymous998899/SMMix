...
def get_args_parser():
    ...
    parser.add_argument('--min_side_ratio', type=float, default=0.25,
...
def main(args):
    ...
    mixup_fn = None
    mixup_active = args.mixup > 0 or args.max_side_ratio > 0.
    if mixup_active:
        from smmix import SMMix
        mixup_fn = SMMix(
            mixup_alpha=args.mixup, prob=args.mixup_prob, switch_prob=args.mixup_switch_prob, mode=args.mixup_mode,label_smoothing=args.smoothing, 
            num_classes=args.nb_classes, min_side_ratio=args.min_side_ratio,max_side_ratio=args.max_side_ratio,side=args.side)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if utils.is_main_process() and args.wandb_project:
            wandb.log({
                'epoch': epoch, 
                'Training Loss': log_stats['train_loss'],
                'Validation Loss': log_stats['test_loss'],
                'Validation Prec@1': log_stats['test_acc1'],
                'Validation Prec@5': log_stats['test_acc5'],
            })
