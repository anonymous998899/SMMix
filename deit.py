...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, return_attn=return_attn)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=None, return_attn=False):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, return_attn=True and i==depth-1)
        ...
        self.token_head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x, target_mask=None, source_mask=None):
        B = x.shape[0]
        ...
        x = self.blocks(x)
        x, attn = x[0], x[1]
        attn = attn[:, :, 1:, 1:].mean(dim=(1,2)) 

        x = self.norm(x)
        class_token = x[:, 0]
        image_token = x[:, 1:]
        class_result = self.head(class_token)
        if not target_mask is None:
            target_result = self.token_head((image_token*target_mask.unsqueeze(-1)).sum(dim=1)/target_mask.sum(dim=1, keepdim=True))
            source_result = self.token_head((image_token*source_mask.unsqueeze(-1)).sum(dim=1)/source_mask.sum(dim=1,keepdim=True))
            return class_result, target_result, source_result
        else:
            return class_result,attn
